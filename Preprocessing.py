...
def preprocess(data):
    data=data.split('\n')
    data=' \n'.join([i.replace('\u202f',' ') for i in data])
    data=data.split('\n')
    pattern = '\d{1,2}/\d{1,2}/\d{2},\s\d{1,2}:\d{1,2}\s\w{1,2}\s-\s'
    messages = re.split(pattern, str(data))[1:]
    dates = re.findall(pattern,str(data))
    ...
    df['Date']=df.Date.apply(lambda x:x.replace('AM','') )
    df['Date']=df.Date.apply(lambda x:x.replace('PM','') )
